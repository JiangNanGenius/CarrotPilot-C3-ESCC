...
def create_acc_commands(packer, enabled, accel, jerk, idx, hud_control, set_speed, stopping, long_override, use_fca, CP, CS, soft_hold_mode, escc=False):
  from opendbc.car.hyundai.carcontroller import HyundaiJerk
  cruise_available = CS.out.cruiseState.available
  soft_hold_active = CS.softHoldActive
  soft_hold_info = soft_hold_active > 1 and enabled
  ...
  if not use_fca:
    ...
    scc12_values["AEB_Status"] = 2 if escc else 1  # AEB disabled unless ESCC is preserving stock warning state
    if escc:
...
def create_acc_opt(packer, CP, escc=False):
  commands = []
