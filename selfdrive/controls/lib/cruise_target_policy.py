...
class CruiseTargetPolicy:
  ...
  def update(self, *, ceiling, road_limit, speed, gas, brake, engaged, standstill):
    ...
    reset = not valid_ceiling or not engaged or brake or standstill or changed
    if reset:
      self.override = None
    elif self.gas_previous and not gas and math.isfinite(speed) and speed > 0:
      self.override = min(speed, ceiling)
    ...
    if valid_ceiling and self.override is not None:
