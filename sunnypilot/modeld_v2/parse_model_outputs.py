...
def _infer_mhp(slice_size: int, prod_out_shape: int, max_in_n: int = 16, max_out_n: int = 6) -> tuple[int, int]:
  for out_n in range(max_out_n + 1):
    per_hypothesis = 2 * prod_out_shape + out_n
    if per_hypothesis <= 0:
      continue
    if slice_size % per_hypothesis == 0:
      in_n = slice_size // per_hypothesis
      if 1 <= in_n <= max_in_n:
        return in_n, out_n
  return 1, 0
...
class Parser:
  ...
  def parse_mdn(self, name, outs, out_shape, in_N=0, out_N=0):
    ...
    if out_N > 1 or (in_N > 1 and out_N == 0):
      selection_count = out_N if out_N > 1 else in_N
      final_shape = tuple([raw.shape[0], selection_count] + list(out_shape))
    else:
